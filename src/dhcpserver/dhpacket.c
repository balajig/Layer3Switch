...
dhcp (struct packet *packet)
{
         ...
         packet->options->universes[agent_universe.index] == NULL))
    {
        ...
        if (packet->options->universe_count <= agent_universe.index)
            packet->options->universe_count = agent_universe.index + 1;
  ...
  nolease:
     ...
     */
    if ((oc = lookup_option (&dhcp_universe, packet->options, DHO_HOST_NAME)))
    {
        if (!oc->expression)
     ...
     int                 ms_nulltp;
{
    ...
    if (!lease)
    {
        if (!allocate_lease (&lease, packet,
                             packet->shared_network->pools, &peer_has_leases))
        {
            if (peer_has_leases)
                log_error ("%s: peer holds all free leases", msgbuf);
            else
        ...
        !load_balance_mine (packet, peer))
    {
        if (peer_has_leases)
        {
            log_debug ("%s: load balance to peer %s", msgbuf, peer->name);
            goto out;
        }
        else
        {
...
void
dhcprequest (packet, ms_nulltp, ip_lease)
     struct packet      *packet;
     int                 ms_nulltp;
     struct lease       *ip_lease;
{
        ...
        (have_requested_addr && !packet->raw->ciaddr.s_addr))
    {
        ...
        if (!find_grouped_subnet (&subnet, packet->shared_network, cip, MDL))
        {
     ...
     int                 ms_nulltp;
{
    ...
    snprintf (msgbuf, sizeof msgbuf,
              ...
              packet->raw->giaddr.s_addr
              ? inet_ntoa (packet->raw->giaddr)
              : packet->interface->name, lease ? "" : "not ");
    ...
    if (lease && lease->pool && lease->pool->failover_peer)
    {
        ...
        if (peer->service_state == not_responding ||
            peer->service_state == service_startup)
        {
            log_info ("%s: ignored%s", peer->name, peer->nrr);
            goto out;
        }
     ...
     int                 ms_nulltp;
{
    ...
    if (lease)
        execute_statements_in_scope ((struct binding_value **) 0,
                                     ...
                                     &global_scope,
                                     lease->subnet->group, (struct group *) 0);
     ...
     int                 ms_nulltp;
{
                               ...
                               &global_scope, oc, MDL))
    {
        ...
        if (i >= sizeof (raw.file))
        {
            log_info ("file name longer than packet field "
                      ...
                      (unsigned long) sizeof (raw.file), i, i, d1.data);
            i = sizeof (raw.file);
        }
        else
            raw.file[i] = 0;
                               ...
                               &global_scope, oc, MDL))
    {
        ...
        if (i >= sizeof (raw.sname))
        {
            log_info ("server name longer than packet field "
                      "truncated - field: %lu name: %d %.*s",
                      (unsigned long) sizeof (raw.sname), i, i, d1.data);
            i = sizeof (raw.sname);
        }
        else
            raw.sname[i] = 0;
    ...
    {
        if (make_const_data (&oc->expression, &dhcpack, 1, 0, 0, MDL))
        {
            option_code_hash_lookup (&oc->option,
                                     dhcp_universe.code_hash, &i, 0, MDL);
    ...
    if ((oc = lookup_option (&server_universe, options, SV_NEXT_SERVER)))
    {
    ...
    outgoing.packet_length =
        cons_options (packet, outgoing.raw, (struct lease *) 0,
                      ...
                      0, nulltp, 0,
                      prl.len ? &prl : (struct data_string *) 0, (char *) 0);
    option_state_dereference (&options, MDL);
     ...
     struct iaddr       *cip;
{
    ...
    if (!make_const_data (&oc->expression, &nak, sizeof nak, 0, 0, MDL))
    {
    ...
    i = DHO_DHCP_MESSAGE_TYPE;
    option_code_hash_lookup (&oc->option, dhcp_universe.code_hash, &i, 0, MDL);
    save_option (&dhcp_universe, options, oc);
    option_cache_dereference (&oc, MDL);
...
void
ack_lease (packet, lease, offer, when, msg, ms_nulltp, hp)
     struct packet      *packet;
     struct lease       *lease;
     unsigned int        offer;
     TIME                when;
     ...
     struct host_decl   *hp;
{
    ...
    for (i = packet->class_count; i > 0; i--)
    {
        execute_statements_in_scope
            ((struct binding_value **) 0,
             ...
             (lease->pool ? lease->pool->group : lease->subnet->group));
    }
    ...
    if (packet->packet_type == DHCPREQUEST &&
        ...
        evaluate_boolean_option_cache (&ignorep,
                                       ...
                                       (struct client_state *) 0,
                                       packet->options,
                                       state->options, &lease->scope, oc, MDL))
    {
        struct lease       *seek;
        if (lease->uid_len)
        {
            ...
            {
                seek = (struct lease *) 0;
                find_lease_by_uid (&seek, lease->uid, lease->uid_len, MDL);
                if (!seek)
                    break;
    ...
    memset (&d1, 0, sizeof d1);
    if (offer == DHCPOFFER &&
        (oc = lookup_option (&server_universe, state->options, SV_MIN_SECS)))
    {
        if (evaluate_option_cache (&d1, packet, lease,
                                   ...
                                   &lease->scope, oc, MDL))
        {
            if (d1.len && ntohs (packet->raw->secs) < d1.data[0])
            {
    ...
    if (!host)
    {
        ...
        if (!host)
        {
            find_hosts_by_haddr (&hp,
                                 packet->raw->htype,
                                 packet->raw->chaddr, packet->raw->hlen, MDL);
            for (h = hp; h; h = h->n_ipaddr)
            {
                if (!h->fixed_addr)
                    break;
    ...
    oc = lookup_option (&server_universe, state->options, SV_ALLOW_BOOTING);
    if (oc &&
        !evaluate_boolean_option_cache (&ignorep,
                                        packet, lease,
                                        (struct client_state *) 0,
    ...
    if (have_billing_classes && !(lease->flags & STATIC_LEASE))
    {
        ...
        if (lease->billing_class)
        {
            for (i = 0; i < packet->class_count; i++)
                if (packet->classes[i] == lease->billing_class)
                    break;
        ...
        if (lease->billing_class == NULL)
        {
            ...
            for (i = 0; i < packet->class_count; i++)
            {
                ...
                {
                    bill++;
                    if (bill_class (lease, packet->classes[i]))
                        break;
                }
            }
            if (bill != 0 && i == packet->class_count)
            {
                log_info ("%s: no available billing: lease "
                          "limit reached in all matching " "classes", msg);
                free_lease_state (state, MDL);
                if (host)
                    host_dereference (&host, MDL);
                return;
    ...
    /* Choose a server name as above. */
    oc = lookup_option (&server_universe, state->options, SV_SERVER_NAME);
    if (oc)
    ...
    if (offer)
    {
                                 ...
                                 SV_DEFAULT_LEASE_TIME)))
        {
                                       ...
                                       state->options, &lease->scope, oc, MDL))
            {
                if (d1.len == sizeof (u_int32_t))
                    default_lease_time = getULong (d1.data);
                data_string_forget (&d1, MDL);
            }
        }
                                               ...
                                               oc, MDL))
            {
                ...
                if (!ignorep)
                    log_info ("Infinite-leasetime "
                              "reservation made on %s.", piaddr (lt->ip_addr));
            }
                                 ...
                                 SV_MAX_LEASE_TIME)))
        {
            ...
            {
                if (d1.len == sizeof (u_int32_t))
                    max_lease_time = getULong (d1.data);
                data_string_forget (&d1, MDL);
            }
        }
        ...
        if (lease->pool &&
            ...
            evaluate_option_cache (&d1, packet, lease, NULL,
                                   ...
                                   &lease->scope, oc, MDL))
        {
            if (d1.len == 1 && d1.data[0] > 0 && d1.data[0] < 100)
            {
                TIME                adaptive_time;
                ...
                poolfilled = (total > (INT_MAX / 100)) ?
                    total / (count / 100) : (total * 100) / count;
                ...
                log_debug ("Adap-lease: Total: %d, Free: %d, "
                           ...
                           (int) adaptive_time, poolfilled, d1.data[0]);

                if (poolfilled >= d1.data[0] && lease_time > adaptive_time)
                {
                    log_info ("Pool over threshold, time "
                              "for %s reduced from %d to "
                              "%d.", piaddr (lease->ip_addr),
                              (int) lease_time, (int) adaptive_time);
        ...
        if (lease->pool && lease->pool->failover_peer)
        {
            ...
            if (lease_time > peer->mclt)
            {
                ...
                if (lt->tsfp <= cur_time)
                    ...
                    new_lease_time = (lt->tsfp - cur_time) + peer->mclt;
            }
            ...
            if (offer == DHCPACK)
            {
                lt->tstp = cur_time + lease_time + (new_lease_time / 2);
    ...
    if (lease->billing_class)
        class_reference (&lt->billing_class, lease->billing_class, MDL);
                               ...
                               NULL, &lease->scope, oc, MDL))
    {
        if (d1.len != 0)
        {
            bind_ds_value (&lease->scope, "vendor-class-identifier", &d1);
        }
    ...
    if (lease->on_commit && (!offer || offer == DHCPACK))
    {
        execute_statements ((struct binding_value **) 0,
                            ...
                            packet->options,
                            state->options, &lt->scope, lease->on_commit);
        if (lease->on_commit)
            executable_statement_dereference (&lease->on_commit, MDL);
    }
    ...
    if (lease->flags & STATIC_LEASE)
    {
        ...
        lease->hardware_addr.hlen = packet->raw->hlen + 1;
        lease->hardware_addr.hbuf[0] = packet->raw->htype;
        memcpy (&lease->hardware_addr.hbuf[1], packet->raw->chaddr, sizeof packet->raw->chaddr);    /* XXX */
    }
    else
    {
         ...
         */
        if (!supersede_lease (lease, lt, 0, !offer || offer == DHCPACK, 0))
        {
#endif
                             ...
                             DHO_SUBNET_SELECTION)))
    {
        ...
        if (option_cache_allocate (&noc, MDL))
        {
            ...
            if (oc->expression)
                expression_reference (&noc->expression, oc->expression, MDL);
            if (oc->option)
    ...
    if (state->offer)
    {
        ...
        if (option_cache_allocate (&oc, MDL))
        {
            if (make_const_data (&oc->expression, &state->offer, 1, 0, 0, MDL))
            {
                option_code_hash_lookup (&oc->option,
                                         dhcp_universe.code_hash, &i, 0, MDL);
        ...
        offered_lease_time = state->offered_expiry - cur_time;
        ...
        if (option_cache_allocate (&oc, MDL))
        {
            if (make_const_data (&oc->expression, state->expiry, 4, 0, 0, MDL))
            {
                option_code_hash_lookup (&oc->option,
                                         dhcp_universe.code_hash, &i, 0, MDL);
                save_option (&dhcp_universe, state->options, oc);
    ...
    else
    {
        ...
        if (state->ip->address_count)
        {
            ...
            memcpy (state->from.iabuf,
                    &state->ip->addresses[0], state->from.len);
        }
    }

    /* Figure out the address of the boot file server. */
    memset (&state->siaddr, 0, sizeof state->siaddr);
    if ((oc = lookup_option (&server_universe, state->options, SV_NEXT_SERVER)))
    {
        if (evaluate_option_cache (&d1, packet, lease,
                                   (struct client_state *) 0,
                                   packet->options, state->options,
    ...
    if (!lookup_option (&dhcp_universe, state->options, i))
    {
        ...
        if (option_cache_allocate (&oc, MDL))
        {
            if (make_const_data (&oc->expression,
                                 ...
                                 lease->subnet->netmask.len, 0, 0, MDL))
            {
                option_code_hash_lookup (&oc->option,
                                         dhcp_universe.code_hash, &i, 0, MDL);
                save_option (&dhcp_universe, state->options, oc);
          ...
          lookup_option (&server_universe, state->options, j), MDL)))
    {
        ...
        if (option_cache_allocate (&oc, MDL))
        {
            if (make_const_data (&oc->expression,
                                 ((unsigned char *)
                                  lease->host->name),
                                 strlen (lease->host->name), 1, 0, MDL))
            {
                option_code_hash_lookup (&oc->option,
                                         dhcp_universe.code_hash, &i, 0, MDL);
                save_option (&dhcp_universe, state->options, oc);
                                        ...
                                        &lease->scope, oc, MDL)))
    {
        ...
        add_timeout (&tv, lease_ping_timeout, lease,
                     (tvref_t) lease_reference, (tvunref_t) lease_dereference);
        ++outstanding_pings;
    }
    else
    {
        lease->cltt = cur_time;
#if defined(DELAYED_ACK)
        if (!(lease->flags & STATIC_LEASE) && (!offer || (offer == DHCPACK)))
...
delayed_ack_enqueue (struct lease *lease)
{
    ...
    {
        q = ((struct leasequeue *) dmalloc (sizeof (struct leasequeue), MDL));
        if (!q)
    ...
    else
    {
        ...
        if (max_fsync.tv_sec == 0 && max_fsync.tv_usec == 0)
        {
            ...
            max_fsync.tv_usec = cur_tv.tv_usec + max_ack_delay_usecs;
     ...
     struct lease       *lease;
{
    ...
    if (state->server_name.len && state->server_name.data)
    {
        ...
        if (sizeof raw.sname > state->server_name.len)
            ...
            log_info ("server name longer than packet field "
                      ...
                      state->server_name.len, state->server_name.data);
    }
    else
        bufs |= 2;                /* XXX */

    memcpy (raw.chaddr, &lease->hardware_addr.hbuf[1], sizeof raw.chaddr);
    ...
    packet_length = cons_options (state->packet, &raw, lease,
                                  ...
                                  bufs, nulltp, bootpp,
                                  &state->parameter_request_list, (char *) 0);
    ...
    log_info ("%s on %s to %s %s%s%svia %s",
              ...
              s ? "(" : "", s ? s : "", s ? ") " : "",
              (state->giaddr.s_addr
               ? inet_ntoa (state->giaddr) : state->ip->name));
           ...
           code. */
             ...
             !((state->got_server_identifier ||
               ...
               (state->shared_network ==
                lease->subnet->shared_network)) && state->offer == DHCPACK)
    {
    ...
    result = send_packet (state->ip,
                          ...
                          from, &to, unicastp ? &hto : (struct hardware *) 0);
            ...
            const char *file, int line)
{
    ...
    if (!fixed_lease && !host)
    {
        if (find_hosts_by_option (&hp, packet, packet->options, MDL) == 1)
        {
            packet->known = 1;
            if (host)
                host_dereference (&host, MDL);
    ...
    while (uid_lease)
    {
        ...
        {
#if defined (DEBUG_FIND_LEASE)
            log_info ("wrong network segment: %s", piaddr (uid_lease->ip_addr));
#endif
            goto n_uid;
        }
        ...
        {
#if defined (DEBUG_FIND_LEASE)
            log_info ("not permitted: %s", piaddr (uid_lease->ip_addr));
#endif
          n_uid:
            if (uid_lease->n_uid)
    ...
    while (hw_lease)
    {
        ...
        if (hw_lease->binding_state != FTS_FREE &&
            ...
            (!have_client_identifier ||
             ...
             memcmp (hw_lease->uid, client_identifier.data, hw_lease->uid_len)))
        {
#if defined (DEBUG_FIND_LEASE)
        ...
        {
#if defined (DEBUG_FIND_LEASE)
            log_info ("wrong network segment: %s", piaddr (hw_lease->ip_addr));
#endif
            goto n_hw;
        }
        ...
        {
#if defined (DEBUG_FIND_LEASE)
            log_info ("not permitted: %s", piaddr (hw_lease->ip_addr));
#endif
            if (!packet->raw->ciaddr.s_addr)
        ...
        ip_lease->uid && ip_lease != uid_lease)
    {
        if (have_client_identifier &&
            (ip_lease->uid_len == client_identifier.len) &&
            !memcmp (client_identifier.data, ip_lease->uid, ip_lease->uid_len))
        {
            if (uid_lease)
            {
                if (uid_lease->binding_state == FTS_ACTIVE)
        ...
        if (packet->packet_type == DHCPREQUEST && fixed_lease)
        {
          ...
          db_conflict:
            ...
            if (fixed_lease)
                lease_dereference (&ip_lease, MDL);
            strcpy (dhcp_message, "database conflict - call for help!");
        }
    ...
    if (hw_lease)
    {
        ...
        else
        {
            ...
            {
#if defined (DEBUG_FIND_LEASE)
                log_info ("not choosing hardware lease: %s.", "uid mismatch");
#endif
            }
        }
        lease_dereference (&hw_lease, MDL);
    ...
    if (lease &&
        ...
        lease == ip_lease && packet->packet_type == DHCPREQUEST)
    {
  ...
  out:
    ...
    {
#if defined (DEBUG_FIND_LEASE)
        log_info ("Returning lease: %s.", piaddr (lease->ip_addr));
#endif
...
int
mockup_lease (struct lease **lp, struct packet *packet,
              struct shared_network *share, struct host_decl *hp)
{
    ...
    if (!find_host_for_network (&lease->subnet, &rhp, &lease->ip_addr, share))
    {
        lease_dereference (&lease, MDL);
            ...
            if (pool->failover_peer->i_am == primary)
            {
                ...
                if (pool->backup != NULL)
                {
                    if (((candl == NULL) ||
                         (candl->ends >
                          pool->backup->ends)) &&
                        lease_mine_to_reallocate (pool->backup))
                    {
                        candl = pool->backup;
                    }
                    else
            ...
            else
            {
                ...
                if (pool->free != NULL)
                {
                    if (((candl == NULL) ||
                         (candl->ends >
                          pool->free->ends)) &&
                        lease_mine_to_reallocate (pool->free))
                    {
                        candl = pool->free;
                    }
                    else
    ...
    if (lease != NULL)
    {
        ...
        if (lease->host != NULL)
        {
            log_debug ("soft impossible condition (%s:%d): stale "
                       ...
                       lease->host->name, piaddr (lease->ip_addr));
            host_dereference (&lease->host, MDL);
        }
            ...
            case permit_class:
                for (i = 0; i < packet->class_count; i++)
                {
                    ...
                    if (packet->classes[i] &&
                        ...
                        (packet->classes[i]->superclass == p->class))
                        return 1;
                }
                break;
                           ...
                           struct option_state *options, struct packet *packet)
{
    ...
    if (packet->interface->address_count > 0)
    {
        if (option_cache_allocate (&oc, MDL))
        {
            ...
            if (make_const_data (&oc->expression,
                                 (unsigned char *) a, sizeof (*a), 0, 0, MDL))
            {
                option_code_hash_lookup (&oc->option,
                                         dhcp_universe.code_hash,
                                         &option_num, 0, MDL);
...
find_min_site_code (struct universe *u)
{
    ...
    if (site_code_min < 224)
    {
        log_error ("WARNING: site-local option codes less than 224 have "
                   ...
                   "or migrated up past 224.", u->name, site_code_min);
    }
