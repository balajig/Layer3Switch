...
void * dhcp_task (void *arg)
{
	...
	void (*dhcp_msg_call_back[]) (struct interface *, void *p) = {NULL, dhcp_process};
	...
	while (1) {
		...
		if (dhcp_msg_call_back [msg->msg_type])
			dhcp_msg_call_back[msg->msg_type] (msg->netif, msg->data);
